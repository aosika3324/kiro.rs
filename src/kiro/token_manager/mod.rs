...
// 上游只读/写 REST 查询方法簇（profileArn 解析 / getUsageLimits / ListAvailableModels /
// setUserPreference）：从主 impl 拆出为同类型的第二个 impl 块，行为零变化。见该文件头注释。
mod remote_api;
// 凭据 CRUD 方法簇（Admin 增删改查 / 分组 / refreshToken 更新）：同上，拆为第二个 impl 块。
...
async fn refresh_external_idp_token(
    ...
    let status = response.status();
    // rate-limit 全链路透传：上游 429 携带 Retry-After 时直接把限流信号返回给调用方。
    let rate_limit_error = (status.as_u16() == 429)
        .then(|| UpstreamRateLimitError::from_headers(response.headers()));
    if !status.is_success() {
        let body_text = response.text().await.unwrap_or_default();
...
pub(crate) async fn get_usage_limits(
    ...
    for (idx, region) in candidates.iter().enumerate() {
        // 区域路由（fork 特性）：external_idp 走 Microsoft/Entra 主机族，其余走 q.{region}。
        let host = if is_external_idp {
            crate::kiro::model::credentials::external_idp_host(region)
        } else {
            format!("q.{}.amazonaws.com", region)
        };
        // URL 构造采用上游修复：这些 REST 调用不再附带 profileArn（Kiro 0.9.2 上会被判为
        // malformed request），profileArn 仅用于流式端点。
...
pub(crate) async fn get_available_models(
    ...
    for (idx, region) in candidates.iter().enumerate() {
        ...
        // URL 构造采用上游修复：不再附带 profileArn（避免 Kiro 0.9.2 判为 malformed）。
...
/// # 锁层级与获取顺序（务必遵守，防死锁）
///
/// 本结构有多把独立的 `parking_lot::Mutex`（`entries`/`metrics`/`credential_locks`/
/// `proxy`/…）与按凭据分的 `TokioMutex`（`refresh_locks`）。它们各自独立，靠**固定的获取
/// 顺序**避免死锁——修改任何同时持有 ≥2 把锁的代码前，必须对齐既有顺序：
///
/// - `ranked_available_credentials` 等调度路径：先锁 `entries`，再锁 `metrics`。
/// - `CredentialLease::drop`：在 `resize_lock` 内锁 `metrics`。
/// - 绝不在持有 `entries` 时反向去抢一个会先抢 `metrics` 的路径，反之亦然。
///
/// 经验规则：**先 `entries` 后 `metrics`**；`refresh_locks`（TokioMutex）只在 async 刷新路径
/// 单独持有，不与上述 parking_lot 锁嵌套跨 await。这是全文件最易引入死锁处，评审时重点看。
...
impl MultiTokenManager {
    ...
    /// 获取指定分组的凭据总数（group=None 时等于 total_count，即返回全部凭据数）
    ///
    ...
    ///
    /// 与 [`Self::report_account_throttled_for_request`] 共用 `throttled_until` 排除通路（被冷却的凭据
