...
pub(crate) fn extract_invoke_content_blocks(
    ...
    loop {
            ...
            && salvaged_input.is_some()
        {
            ...
            // 真泄漏：用 fork 的严格入参（salvaged_input，半截/非法 JSON 已在守卫处降级为文本），
            // 名字保留 Kiro 侧短名传给 from_kiro —— from_kiro 内部统一还原「名字 + 入参」
            // （与结构化 / websearch 路径同口径），故此处不得预还原名字，否则会双重还原、
            // 且入参反向重写会因名字不匹配而失效。
            ...
            // 观测日志需客户端可见名（长名 >63 会被缩短，模型可能泄漏短名）；
            // 仅用于 record_salvaged_leak，真正的还原由下方 from_kiro 完成。
...
impl StreamContext {
                    ...
                    match find_invoke_block_end(&buf, start) {
                        Some(end) => {
                                ...
                                && salvaged_input.is_some()
                            {
                                ...
                                let (name, _) = parsed.expect("parsed is Some when name_known");
                                // fork：严格入参（半截/非法 JSON 已在守卫处降级为文本）。
                                let input =
                                    salvaged_input.expect("salvaged_input is Some (checked in guard)");
                                // fork 观测：记录还原后的客户端工具名（仅用于 leak 日志）。
                                let client_name = self
                                    ...
                                    .unwrap_or_else(|| name.clone());
                                record_salvaged_leak(&client_name);
                                // upstream：统一经 from_kiro 还原「名字 + 入参」并从 emit_completed_tool_use
                                // 单一出口发出（与结构化 toolUseEvent 同口径）。name 保留 Kiro 短名。
                                let tool_use_id =
    ...
    pub fn generate_final_events(&mut self) -> Vec<SseEvent> {
        ...
        // 中断收尾（fork）:跳过 thinking / invoke-sniff / tool_use-XML 缓冲的 flush。
        // 这些缓冲此刻可能持有半截的 `<invoke …>` / `</thinking>` / `<tool_use>` 标签
        // (等后续 chunk 才能判定),中断时把它们当正文吐出会让下游看到伪造的标签碎片。
        // 直接走 state_manager 收尾发可重试 `error` 事件(不伪造 end_turn)。
        ...
        // 正常收尾（upstream）：flush <tool_use> XML 过滤器的残留（截断的未闭合块会被丢弃），
