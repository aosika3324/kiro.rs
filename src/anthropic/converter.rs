...
fn normalize_json_schema(schema: serde_json::Value) -> serde_json::Value {
    ...
    // 把各分支 object 的 properties 合并到顶层 properties（fork 的 union 语义，避免丢入参），
    // 再剥离组合器关键字，保证顶层是干净的 type:object 形态。嵌套（property 级）的组合器
    // 上游接受，由 normalize_property_schema 处理，此处不动。
    //
    // required：若原 schema 顶层未显式声明，则从【首个 type=object 且带 required 的 variant】
    // 恢复（对齐上游 strip_top_level_combinators 的字段恢复，保留主分支的语义约束，
    // 而非把 union 整体降级为无 required）。空/无 required 仍由后续归一逻辑省略。
    let had_required_initially = obj.contains_key("required");
    let mut recovered_required: Option<serde_json::Value> = None;
    for key in ["oneOf", "allOf", "anyOf"] {
        ...
        for branch in branches {
            ...
            if !had_required_initially
                && recovered_required.is_none()
                && bobj.get("type").and_then(|v| v.as_str()) == Some("object")
            {
                if let Some(req @ serde_json::Value::Array(_)) = bobj.get("required") {
                    recovered_required = Some(req.clone());
                }
            }
    ...
    if let Some(req) = recovered_required {
        obj.entry("required".to_string()).or_insert(req);
    }

    // type 顶层必须是 "object"（Bedrock ToolInputSchema 硬约束）；非 object 一律【强制】修正。
    // 上游对 array/string 等顶层 type 返回 TOOL_SCHEMA_INVALID 400，仅补缺失不够，须覆盖已有值。
...
///
/// 注:合并解冲突后 `normalize_json_schema` 采用 fork 的顶层内联合并逻辑(union 各分支 properties),
/// 不再调用本函数;保留供后续参考/复用,故标注 dead_code。
#[allow(dead_code)]
...
pub fn map_model(model: &str) -> Option<String> {
    ...
    if model_lower.contains("fable") {
        ...
        {
            // Sonnet 5（1M 上下文，实验预览）。精确匹配 5 代，放在 4.x 分支之后，
            // 避免命中 legacy claude-3-5-sonnet；上游 ListAvailableModels 的 modelId 即 "claude-sonnet-5"。
        ...
        // GPT-5.x 直通已在函数开头 is_passthrough_upstream_model 精确放行
        // （gpt-5.6-sol / terra / luna）。裸 "gpt-5.6"（无子型号后缀）不是上游合法 id，
        // 不做松前缀放行，返回 None，避免下发一个上游会 400 的模型。
        None
    }
}
    ...
    let raw = req
        ...
        .or_else(|| {
            req.thinking
                ...
                .map(|t| effort_from_budget_tokens(t.budget_tokens).as_str().to_string())
        })
...
/// 便捷入口：默认按 ClaudeCode 模式转换。payload_truncate 在预算测量时按此口径运行。
                    ...
                    match block.block_type.as_str() {
                        ...
                        "tool_use" => {
                            if let (Some(id), Some(name)) = (block.id, block.name) {
                                // fork（Bug B 修复）：Bedrock/Kiro 要求 toolUses[].input 必须是
                                // JSON 对象；非对象（null / 字符串 / 数组 / 数字，或显式 "input": null）
                                // 会被上游拒为 "Invalid tool use format" (REQUEST_BODY_INVALID)。
                                // 对齐 Kiro-Go：仅当 input 为对象时保留，否则一律降级为 {}。先做此守卫，
                                // 保证进入下方 upstream 入参重写的一定是对象。
                                let input = match block.input {
                                    Some(v) if v.is_object() => v,
                                    _ => serde_json::json!({}),
                                };
                                // upstream：mode-aware 名字映射（ClaudeCode 命中内置改名并记映射，
                                // 否则回退 map_tool_name 的清洗/缩短逻辑）。
                                let mapped_name =
                                    map_client_tool_name_to_kiro(&name, tool_name_map, mode);
                                // upstream：入参按内置工具键位重写（Raw / 非内置直通）。
...
mod tests {
    ...
    fn test_bash_write_edit_description_suffixes_applied() {
        ...
        // Raw 模式：保留客户端原始工具名并走「非内置」后缀分支（本测试按原始名断言后缀）。
        let converted = convert_tools(&tools, &mut map, ToolCompatibilityMode::Raw).unwrap();
        let by = |n: &str| {
            converted
                .iter()
    ...
    fn test_tool_use_non_object_input_coerced_to_object() {
        ...
        for bad in cases {
            ...
            let out = convert_assistant_message(&msg, &mut map, ToolCompatibilityMode::Raw).unwrap();
            let tus = out
                .assistant_response_message
                .tool_uses
    ...
    fn effort_from_budget_tokens_maps_tiers() {
        assert_eq!(effort_from_budget_tokens(2_000).as_str(), "low");
        assert_eq!(effort_from_budget_tokens(4_000).as_str(), "low");
        assert_eq!(effort_from_budget_tokens(10_000).as_str(), "medium");
        assert_eq!(effort_from_budget_tokens(16_000).as_str(), "medium");
        assert_eq!(effort_from_budget_tokens(20_000).as_str(), "high");
        assert_eq!(effort_from_budget_tokens(64_000).as_str(), "high");
        assert_eq!(effort_from_budget_tokens(100_000).as_str(), "xhigh");
    }
